...
async function post(req, res) {
  try {
    ...
    const user_available = await DatabaseDrivpass.drivpass_user_type(
      id_driver,
      1
    );
    if (user_available == null)
      return res
        .status(200)
        .json({ success: false, message: "User is not a driver!" });

    const car = await DatabaseCarInfo.selectPlate(plate);
    if (car == null)
      return res
        .status(200)
        .json({ success: false, message: "Code driver no found!" });
    ...
    const post = await DatabaseDrivpass.insert(refer, {
      id_driver: id_driver,
      id_driver_refer: id_refer
    });
    return res
      .status(200)
      .json({ success: true, message: "Code driver added" });
  } catch (e) {
    console.log(e);
